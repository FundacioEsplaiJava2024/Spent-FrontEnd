export const nameValidator = (value: string, enabled: boolean = true) => {
    ...
    if (value.length < 1) return "The name must have at least 3 characters";
...
export const passwordValidator = (value: string, enabled: boolean = true) => {
    if (!enabled) return false;
    if (value.length < 8) return "The password must have at least 8 characters";
    else if (value.length > 15) return "The password must have fewer than 15 characters";
